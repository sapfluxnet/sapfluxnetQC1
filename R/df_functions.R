...
################################################################################
#' Interactively select the data sets ready for the units transformation
#'
#' Shiny app to flag data sets as ready to move to units transformation
#'
#' @family apps
#'
#' @return An interactive app to flag datasets as ready to move to to units
#'   transformation
#'
#' @export
#'
...
# START
# Function declaration
df_flag_to_units_app <- function(parent_logger = 'test') {
  ...
  withCallingHandlers({
    ...
    shinyApp(
      ...
      ui = fluidPage(
        # app title
        titlePanel('Flag to units transformation app'),
      ...
      server = function(input, output) {
        ...
        pop_tables <- eventReactive(
          ...
          valueExpr = {
            ...
            if (clicked == 0) {
              freeze_list <- df_whos_ready_to('units', filter = 'freeze',
                                              parent_logger = parent_logger)
              ready_list <- df_whos_ready_to('units', filter = 'ready',
                                             parent_logger = parent_logger)
              done_list <- df_whos_ready_to('units', filter = 'done',
                                            parent_logger = parent_logger)
        ...
        output$freeze <- DT::renderDataTable({
          ...
          DT::datatable(
            freeze_table,
            colnames = c('Site', 'Status "TO_UNITS"'),
            extensions = 'Scroller',
            options = list(
              dom = 'ti',
              scrollY = 400,
