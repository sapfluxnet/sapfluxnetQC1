...
site_md_coordfix <- qc_coordinates(site_md, plot = TRUE,
                                   parent_logger = logger_name)

site_md_coordfix <- qc_get_biome(site_md_coordfix,
                                 plot= FALSE,
                                 merge_deserts = FALSE,
                                 parent_logger = logger_name)
...
```{r coordinates_on_land, echo=FALSE}
site_md_coordfix %>%
  dplyr::select(si_name, si_lat, si_long, is_on_land) %>%
  mutate(is_on_land = as.character(is_on_land)) %>%
  datatable(class = 'display compact', rownames = FALSE, filter = 'none',
            extensions = c('Scroller'),
            caption = 'On land',
            options = list(dom = 'ti',
...
Potential errors in TIMESTAMP using average radiation

```{r timestamp_radiation_averages, fig.width=12}
# Function to check column presence
has_column <- function(df, col_name) {
  col_name %in% names(df)
}

if (has_column(env_data_fixed, "sw_in")){
  env_data_fixed %>%
  mutate(hour = lubridate::hour(TIMESTAMP)) %>%
  group_by(hour) %>%
  summarise(
    avg_radiation = mean(sw_in, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  ggplot(aes(x = hour, y = avg_radiation)) +
  geom_line(color = "blue", size = 1) +
  geom_point(color = "red", size = 2) +
  geom_vline(xintercept = 12, color = "red3")+
  scale_x_continuous(breaks = 0:23) +
  labs(
    title = "Hourly Average Radiation",
    x = "Hour of the Day",
    y = "Average sw_in"
  ) +
  theme_minimal() +
  ...
  group_by(hour) %>%
  summarise(
    avg_radiation = mean(ppfd_in, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  ggplot(aes(x = hour, y = avg_radiation)) +
  geom_line(color = "blue", size = 1) +
