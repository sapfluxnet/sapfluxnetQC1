---
title: "Level II report for `r params$code`"
...
# log setup
logger_name <- paste('LVL2', params$code, 'qc_lvl2_process',
                     'rep_sfn_render', sep = '.')
...
```{r lvl2, message=FALSE, warning=FALSE}
# libraries
...
out_to_remove_path <- file.path('Data', params$code, 'Lvl_2', 'lvl_2_out_warn',
                                paste0(params$code, 'out_to_remove.txt'))
...
ranges_to_remove_path <- file.path('Data', params$code, 'Lvl_2', 'lvl_2_out_warn',
                                paste0(params$code, 'ranges_to_remove.txt'))
...
manual_to_remove_path <- file.path('Data', params$code, 'Lvl_2', 'lvl_2_out_warn',
                                paste0(params$code, 'manual_to_remove.txt'))
...
sapf_flags_summary <- data.frame(
  stringsAsFactors = FALSE,
  Flags = c('OUT_REPLACED', 'RANGE_REMOVE', 'MANUAL_REMOVED'),
  TO_REM = c(sapf_out_removed, sapf_ranges_removed, sapf_manual_removed)
) %>%
  dplyr::full_join(sapf_flags_table, by = Flags)
...
env_flags_summary <- data.frame(
  ...
  dplyr::full_join(env_flags_table, by = Flags)
```
