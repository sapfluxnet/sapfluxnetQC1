...
output$sp_bar <- renderggiraph({
  ggiraph(code = {print(sp_bar_plot)},
          width = 0.95, width_svg = 6, height_svg = 6.5, zoom_max = 5,
          tooltip_extra_css = "background-color:black;font-style:italic;padding:5px;border-radius:10px 20px 10px 20px;color:white",
          hover_css = "fill:red")
})

output$sp_leaf_habit <- renderggiraph({
  ggiraph(code = {print(sp_leaf_plot)},
          width = 0.95, width_svg = 6, height_svg = 3.2, zoom_max = 5)
})
