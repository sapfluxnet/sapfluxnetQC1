...
fillRow(height = '100%', flex = 1,
  plotOutput("sp_bar", width = "100%"),
  fillCol(width = "100%", flex = 1,
          plotOutput("sp_leaf_habit", height = "100%"),
          plotOutput("sp_genus", height = "100%"))
)
...
output$sp_leaf_habit <- renderPlot({
  sp_leaf_plot
})

output$sp_genus <- renderPlot({
  sp_genus_plot
})
```
...
fillCol(width = '100%', flex = 1,
  ...
  fillRow(height = "100%", flex = 1,
          plotOutput("st_aspect_terrain", width = "100%"),
          plotOutput("st_density_basal", width = "100%"))
)
...
output$st_aspect_terrain <- renderPlot({
  st_aspect_terrain_plot
})

output$st_density_basal <- renderPlot({
  st_density_basal_area_plot
})
```
...
fillCol(
  ...
  plotOutput('dbh', height = "100%")
)

output$dbh <- renderPlot({
...
output$env_vars <- renderPlot({
  env_vars_plot
})

output$swc <- renderPlot({
  env_swc_plot
})
...
fillCol(
  width = '100%', flex = 1,
  plotOutput('si_biomes')
)

output$si_biomes <- renderPlot({
  si_biomes_plot
})
```
