...
# pics for Height ans Soil Depth plot
# tree <- as.matrix(as.raster(rsvg::rsvg(
#   system.file('pics', 'pine_tree.svg', package = 'ggpic')
# )))
# 
# root <- as.matrix(as.raster(rsvg::rsvg('root.svg')))
...
```{r}
# # si_md file paths
# si_md_paths <- list.files(path = file.path('Data'),
#                            pattern = '_site_md.csv',
#                            full.names = TRUE, recursive = TRUE)
# # data frame with all site metadata
# si_biomes_df <- lapply(si_md_paths, read_csv) %>%
#   bind_rows() %>%
#   select(
#     si_code, si_lat, si_long
#     #, si_mat, si_map
#   )
# 
...
# fillCol(
#   width = '100%', flex = 1,
#   plotlyOutput('si_biomes')
# )
# 
# output$si_biomes <- renderPlotly({
#   ggplotly(si_biomes_plot, tooltip = c('tooltip', 'y', 'x'))
# })
```
...
Here you can:
  ...
  > NOTE: Biomes plot not working at the moment due to porblems in RFc package
