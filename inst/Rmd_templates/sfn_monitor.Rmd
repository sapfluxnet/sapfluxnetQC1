...
get_sapf_data <- reactive({
  read_csv(
    file = file.path('Data', input$site_sel, input$level_sel,
                     paste0(input$site_sel, input$data_sel)),
    col_types = cols(
      TIMESTAMP = col_datetime(),
      .default = col_double()
    )
...
get_sapf_data_vs <- reactive({
  read_csv(
    file = file.path('Data', input$site_sel_vs, input$level_sel_vs,
                     paste0(input$site_sel_vs, input$data_sel_vs)),
    col_types = cols(
      TIMESTAMP = col_datetime(),
      .default = col_double()
    )
...
radioButtons('data_sel', label = 'Select sapflow data type:',
             choices = c('Sapflow data leaf' = '_sapflow_data_leaf.csv',
                         'Sapflow data plant' = '_sapflow_data_plant.csv',
                         'Sapflow data sapwood' = '_sapflow_data_sapwood.csv'),
             selected = '_sapflow_data_plant.csv')
...
output$sapf_ts_vs <- renderDygraph({
  ...
  sapf_plot_vs <- sapf_plot_data_vs %>%
    select(TIMESTAMP, input$plant_sel_vs) %>%
  ...
  dygraph(sapf_plot_vs, group = 'vs') %>%
    ...
    dyCrosshair(direction = 'both') %>%
    dyOptions(useDataTimezone = TRUE, retainDateWindow = TRUE,
              colors = stringr::str_sub(viridis::viridis(1), 1, -3),
              drawGrid = FALSE) %>%
    dyLegend(width = 1000) %>%
    dyRoller(rollPeriod = 1) %>%
    dyRangeSelector()
...
output$env_ts_vs <- renderDygraph({
  ...
  env_plot_vs <- env_plot_data_vs %>%
    select(TIMESTAMP, input$env_sel_vs) %>%
    qc_as_timestamp(env_md_plot_vs, parent_logger = NULL)
